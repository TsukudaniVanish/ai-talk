...
instance FromJSON SupportedFeatures where
  parseJSON = withObject "SupportedFeatures" $ \v ->
    SupportedFeatures
      <$> v
...
instance FromJSON Speaker where
  parseJSON = withObject "Speaker" $ \v ->
    Speaker
      <$> v
      .: "name"
      <*> v
...
instance FromJSON AccentPhrase where
  parseJSON = withObject "AccentPhrase" $ \v ->
    AccentPhrase
      <$> v
      .: "moras"
      <*> v
...
instance FromJSON AudioQuery where
  parseJSON = withObject "AudioQuery" $ \v ->
    AudioQuery
      <$> v
      .: "accent_phrases"
      <*> v
