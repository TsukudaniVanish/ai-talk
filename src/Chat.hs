...
instance ToJSON ChatRequest where
  toJSON (ChatRequest model messages stream) =
    object
      [ "model" .= model,
        "messages" .= messages,
        "stream" .= stream
...
instance ToJSON ChatResponse where
  toJSON (ChatResponse model created_at message done total_duration load_duration prompt_eval_count prompt_eval_duration eval_count eval_duration) =
    object
      [ "model" .= model,
...
instance FromJSON ChatStreamResponse where
  parseJSON = withObject "ChatStreamResponse" $ \v ->
    ChatStreamResponse
      <$> v
      .: "model"
      <*> v
...
instance ToJSON ChatStreamResponse where
  toJSON (ChatStreamResponse model created_at message done total_duration load_duration prompt_eval_count prompt_eval_duration eval_count eval_duration) =
    object
      [ "model" .= model,
        "created_at" .= created_at,
        "message" .= message,
...
chatWithText ollamaModel input =
  chat
    $ ChatRequest
